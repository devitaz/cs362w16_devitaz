...
int scoreFor (int player, struct gameState *state) {
  ...
  //score from deck
  for (i = 0; i < state->discardCount[player]; i++)   // bug-should be deckCount[]
    {
...
int adventurer_card(struct gameState *state,int* temphand,int* z,int* drawntreasure,int cardDrawn)
{
  int currentPlayer = whoseTurn(state);
...
int mine_card(struct gameState *state,int handPos, int choice1, int choice2)
{
  ...
  if ( (getCost(state->hand[currentPlayer][choice1]) + 3) > getCost(choice2) )  // bug-'>' should be '<'
  {
    return -1;
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      adventurer_card(state,temphand,&z,&drawntreasure,cardDrawn);
    ...
    case remodel:
      remodel_card(state,handPos,choice1,choice2);
